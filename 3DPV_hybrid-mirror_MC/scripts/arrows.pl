...
print OUT "t" ."$j = Graphics3D[{Blue,Thick,Arrowheads[Medium],Arrow[{{0,0,0},{$n1,$n2,$n3}}], Boxed->False}]\n" ;
}

$point = $j+1 ;
$cuboid = $j+2;
print OUT "t" ."$point = Graphics3D[{PointSize[Large], Red, Point[{1.0, 0.05, 1.0}]}, Boxed->False]\n" ;
#print OUT "t" ."$cuboid = Graphics3D[{Opacity[0], Cuboid[{0, 0, 0}, {10, 10, 10}]}, Boxed->False]\n";
print OUT "t" ."$point = Graphics3D[{PointSize[Large], Red, Point[{0.0, 0.0, 1.0}]}, Boxed->False]\n" ;
##################
#Write Show line
print OUT "Show[" ;
