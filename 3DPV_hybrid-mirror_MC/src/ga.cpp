 ...
 **============================================================*/
bool GA::nsgaGenerate(double best_parray[], int a_factor, double best_oarray0[], double best_oarray1[], int *my_gen_curr) {
	double *crowdDist;
	int ii, np = 2*(globalSetup->populationSize), fid = 0;
	int numNextGen = 0, indID, *index, *sortedIndex;
	int numEvals;
	//int bestOne=0; // tracks the index of the best individual so far
	...
	/***************************************************************************************/
	// ADDED by BRYAN 11/18: save best individual to an array each generation
	// I had to use public access functions, since this data is protected, can't just point to it
	// first need to find out the best individual
	/*for (int iii = 0; iii < globalSetup->populationSize; iii++) {
		// find the index corresponding to the best individual
		// crowdingComp returns 1 if the first guy wins, 0 if the second guy wins
		if (bestOne!=iii)
		{
			Individual guy10 = Individual(population->guys[bestOne]);
			Individual guy20 = Individual(population->guys[iii]);
			Individual *guy1=&guy10;
			Individual *guy2=&guy20;
			
			if ((((NsgaIndividual*)guy1)->rank < ((NsgaIndividual*)guy2)->rank)|| ((((NsgaIndividual*)guy1)->rank == ((NsgaIndividual*)guy2)->rank)
			&& (((NsgaIndividual*)guy1)->crowdingDistance > ((NsgaIndividual*)guy2)->crowdingDistance)))
			{
			}
			else {
				bestOne=iii;
	...
	for (int iii = 0; iii < (globalSetup->noOfDecisionVariables); iii++) {
		best_parray[genID*a_factor+iii] = (population->guys[bestOne]/*getBestIndividual()*///->getChromosome())->getGene(iii);
	//}
	// , the objective result (best individual objective0)
	//best_oarray0[genID]= (pop->bestobj[0]/*BestIndividual()->getObjective()*/);
	//best_oarray1[genID]= (pop->bestobj[1]/*BestIndividual()->getObjective()*/);
	...
	*my_gen_curr = genID;
	...
	if (!(stoppingCriteria())) {
		...
		for (ii = 0; ii < globalSetup->populationSize; ii++)
			*((NsgaIndividual *)(pop->newGuys[ii]))
					= *(pop->guys[pop->mpool[ii]]);
		pop->doCrossover();
		pop->doMutate();
		...
		numNextGen = fid = 0;
		while (numNextGen + (pop->numIndsFront[fid])
		...
		if (numNextGen < globalSetup->populationSize) {
			...
			for (ii = numNextGen; ii < globalSetup->populationSize; ii++) {
				indID
						= index[sortedIndex[pop->numIndsFront[fid] + numNextGen - ii - 1]];
				*((NsgaIndividual *)(pop->guys[ii]))
						= (*(pop->combinedGuys[indID]));
			}
			delete []index;
			delete []crowdDist;
...
bool GA::generate(double best_parray[], int a_factor, int *my_gen_curr) { // BAM: originally had no argument, added this for data collection
	int ii, numEvals;
	...
	std::cout << "% Generation: " << genID << std::endl;
	//std::cout << (*population) << std::endl; // COMMENTED OUT 3/16/09 cuz its annoying output on SGA
	//
	// Output the population to the history file -- End
	//
	...
	/***************************************************************************************/
	// ADDED by BRYAN 11/18: save best individual to an array each generation
	// not I had to use public access functions, since this data is protected, can't just point to it
	/*for (int iii = 0; iii < (globalSetup->noOfDecisionVariables); iii++) {
		best_parray[genID*a_factor+iii] = (population->getBestIndividual()->getChromosome())->getGene(iii);
	}
	// , the objective result (best individual fitness)
	best_farray[genID]= (population->getBestIndividual()->getObjective());*/ // COMMENTED OUT 3/16/09, not needed for data acquisition
	...
	*my_gen_curr = genID;
	...
	if (!(stoppingCriteria())) {
		...
		if (globalSetup->nichingType != DeterministicCrowding) {
			population->doSelect();
			for (ii = 0; ii < globalSetup->populationSize; ii++)
				*(population->newGuys[ii])
						= *(population->guys[population->mpool[ii]]);
		} else
			for (ii = 0; ii < globalSetup->populationSize; ii++)
		...
		// Fitness Statistics Generation
		population->computeFitnessStatistics();
		if ((globalSetup->selectionType == SUS)|| (globalSetup->selectionType
				== RouletteWheel)|| (globalSetup->nichingType == Sharing)) {
	...
	for (ii = 0; ii < globalSetup->noOfStoppingCriterias; ii++) {
		...
		case ChangeInBestFitness:
			if (genID > globalSetup->genNumWindow)
				if (successiveNoChangeInBestFitness
						== globalSetup->genNumWindow)
					stopFlag = 1;
			if (population->getBestFitChange()
					<= globalSetup->stoppingParameter[ii])
		...
		case ChangeInBestObjective:
			if (genID > globalSetup->genNumWindow) {
				if (successiveNoChangeInBestObjective
						== globalSetup->genNumWindow)
					stopFlag = 1;
