...
//reviewed on Dec 11, Fresnel eqn for parallel part, Bryan's one was wrong. Marco
	...
	//marco//double A = cos(transm_angle)/cos(inc_angle);             // alpha factor in Fresnel's equations
	//marco//double B = relative_permeability*(1/nRatio);            // beta factor in Fresnel's equations
	//(bryan's) double Tcoeff = A*B*pow(2/(A+B),2);			// [0,1] factor for how much transmitted
	//Tcoeff=1; // for testing
	double RP_check = tan(inc_angle-transm_angle)/tan(transm_angle+inc_angle);  				// [0,1] factor for how much reflected
        RP_check = RP_check*RP_check;

	// NEW, for unpolarized light
	double RS = (sin(transm_angle-inc_angle)/sin(transm_angle+inc_angle));
	RS = RS*RS; // square it
	double Rcoeff = (RP_check+RS)/2; // take the average for unpolarized light
	double Tcoeff = 1-Rcoeff;    //variable refed. here, so if changed above no problem
	/////////////////
	// OLD
		//double Rcoeff=RP_check;
	////////////////////////////
		...
		//										}
									...
									if (best_tri_sofar >=0) // there actually exists triangle hit with refl'd ray
									{
										...
										if(reflection) {
											...
											double transm_angle = asin(nRatio*sin(inc_angle));	// snell's law...
											double A = cos(transm_angle)/cos(inc_angle);
											double B = relative_permeability*(1/nRatio);
											double Tcoeff_ref = A*B*pow(2/(A+B),2);
											//Tcoeff_ref=1; // for testing

											double RP_check = 1 - Tcoeff_ref;  				// [0,1] factor for how much reflected
											// /////////NEW
											double RS = (sin(transm_angle-inc_angle)/sin(transm_angle+inc_angle));
											RS = RS*RS; // square it
											double Rcoeff = (RP_check+RS)/2; // take the average for unpolarized light
											Tcoeff_ref = 1-Rcoeff;// new Tcoefficient
											/////////////////
											// OLD//////////
											//double rcoeff=RP_check;
											/////////////////

											curCellReflPower+=reflPowerFlux*efficiency*final_dot*Tcoeff_ref/*(curr_s_triangle->Area)/max_rayfires*/;
...
//	else
//		{printf("time = %f sec, energy = %f WattSec : %f kWh, %f WattSec regained from refl\n", time, totalEnergy, totalEnergy/(3600000), totalEnergyFromRefl);}//SGA OUTPUT
...
//wrap-around version, for mirror and regular cell
...
//wrap-around version, for absorber
double newCoordinate(double oldCoordinate, int dimension) {
	switch(dimension) {
	case 0:
	...
	if(newCoordinate < 0) {
		...
		//x
		case 0:
			ratio1 = length/innerXBound;
			ratio2 = ratio1 - floor(ratio1);
			newCoordinate = innerXBound*(1-ratio2);
			break;
		//y
		case 1:
			ratio1 = length/innerYBound;
			ratio2 = ratio1 - floor(ratio1);
			newCoordinate = innerYBound*(1-ratio2);
			break;
		//z
		case 2:
			ratio1 = length/innerZBound;
			ratio2 = ratio1 - floor(ratio1);
	...
	switch(dimension) {
	case 0:
		newCoordinate += (outerXBound - innerXBound)/2;
		break;
	case 1:
		newCoordinate += (outerYBound - innerYBound)/2;
		break;
	case 2:
		break;
	}
