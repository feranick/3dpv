...
#include "AABBTriangleOverlapTest.hpp"
...
const int MC_numInputs = 5;
int MC_maxSteps;
int MC_numChanges;
double MC_stepSize;
double MC_TValue; //the energy of a box of the same dimension
int TValueType;
double TValueParameter1, TValueParameter2;

//Monte Carlo with mirrors and absorbers variables
const int MC2_numInputs = 11;
int numMirrors, numAbsorbers;
double innerXBound, innerYBound, innerZBound;
string MC_flag;
...
double outerXBound = 1000;  // ground-plane footprint in x (east-west). length is in meters
double outerYBound = 1000;// same for y-direction (north-south)
double Footprint_xmin = 0.0; 
double Footprint_ymin = 0.0;
double outerZBound = 1000;     // ie, maximum height permitted for structure
...
int loadInitPopulation=1;      // true, else if 0 then random initialization
...
const double index_ref_plastic = 1.505;    // values are somewhere between 1.46 to 1.55
...
void GetPairCorrelationFunction(int thisgen)
{
	...
	int num_dist_bins = (int)(sqrt( pow(outerXBound-Footprint_xmin, 2)+
					pow(outerYBound-Footprint_ymin, 2)+
					pow(outerZBound-Structure_zmin, 2))/dr); printf("number of distribution bins is %i\n", num_dist_bins);
	...
	double cell_number_density = (double)numCells / ((outerXBound-Footprint_xmin)*(outerYBound-Footprint_ymin)*(outerZBound-Structure_zmin));
...
{	
	...
	outerXBound			= atof(stdInputs[3]);
	outerYBound			= atof(stdInputs[4]);
	outerZBound				= atof(stdInputs[5]);
	...
	if(mode == 0) { //fixed shape
		ifs.getline(allInputModels[0], 100);
	} else if(mode == 1) { //fixed shape with specs
		ifs.getline(allInputModels[0], 100);
		string s(allInputModels[0]);
		...
		loadInitPopulation		= atoi(GAInputs[6]);
		allow_area_restrict		= atoi(GAInputs[7]);
		GA_numDistinctModels	= atoi(GAInputs[8]);

		if(loadInitPopulation) {
			for(int i = 0; i < GA_numDistinctModels; i++) {
				ifs.getline(allInputModels[i], 100);
			}
		}
	} else if(mode == 3) { //MC
		char MCInputs[MC_numInputs][100];

		for(int i = 0; i < MC_numInputs; i++ ) {
			ifs.getline(MCInputs[i], 100);
		}

		MC_maxSteps				= atoi(MCInputs[0]);
		MC_numChanges			= atoi(MCInputs[1]);
		MC_stepSize				= atof(MCInputs[2]);

		char *TValueInfo = strtok(MCInputs[3], " ");
		if(strcmp(TValueInfo, "const") == 0) {
			TValueType = 0;
		} else if(strcmp(TValueInfo, "power") == 0) {
			TValueType = 1;
		} else {
			TValueType = 2;
		}

		TValueInfo = strtok(NULL, " ");
		TValueParameter1 = atof(TValueInfo);

		TValueInfo = strtok(NULL, " ");
		TValueParameter2 = atof(TValueInfo);

		loadInitPopulation		= atoi(MCInputs[4]);
		if(loadInitPopulation) {
			ifs.getline(allInputModels[0], 100);
			string s(allInputModels[0]);
			modelSpecDir = s.substr(0, (s.length()-4)) + "_c.txt";
		} else {
			modelSpecDir = "random_c.txt";
		}
	} else if(mode == 4) { // mirror and absorber MC
		char MCInputs[MC2_numInputs][100];

		for(int i = 0; i < MC2_numInputs; i++ ) {
			ifs.getline(MCInputs[i], 100);
		}

		numAbsorbers	= atoi(MCInputs[0]);
		numMirrors		= atoi(MCInputs[1]);
		innerXBound		= atof(MCInputs[2]);
		innerYBound		= atof(MCInputs[3]);
		innerZBound		= atof(MCInputs[4]);
		MC_flag			= MCInputs[5];

		MC_maxSteps				= atoi(MCInputs[6]);
		MC_numChanges			= atoi(MCInputs[7]);
		MC_stepSize				= atof(MCInputs[8]);

		char *TValueInfo = strtok(MCInputs[9], " ");
		if(strcmp(TValueInfo, "const") == 0) {
			TValueType = 0;
		} else if(strcmp(TValueInfo, "power") == 0) {
			TValueType = 1;
		...
		loadInitPopulation		= atoi(MCInputs[10]);
		if(loadInitPopulation) {
			ifs.getline(allInputModels[0], 100);
			string s(allInputModels[0]);
			modelSpecDir = s.substr(0, (s.length()-4)) + "_c.txt";
...
// the models. then, retrieveCoordinatesFromFile() is called once for each model specified in the input file
int ReadStructureVertices_Count (int model_ind) 
{
  ...
  strcpy(Clone_Dir, Main_Dir);
	//printf("will count in: %s\n", strcat(Clone_Dir, allInputModels[model_ind]));
	//printf("%s\n", Clone_Dir);
	pFile = fopen (/*Clone_Dir*/strcat(Clone_Dir, allInputModels[model_ind]), /*"rb"*/"r");
...
// retrieveCoordinatesFromFile(): see comment for previous function
...
void retrieveCoordinatesFromFile(double *coordinates, int modelIndex) {
	ifstream ifs(allInputModels[modelIndex]);
	char curLine[300];
	char *coordinate;
	...
	for(int i = 0; i < numCells; i ++) {
		...
		ifs.getline(curLine, 300);
		coordinate = strtok(curLine, " ,\n");
		while(coordinate != NULL) {
			coordinates[9*i + j] = atof(coordinate);
...
//	char * buffer;
//	size_t result;
//	strcpy(Clone_Dir, Main_Dir); printf("%s\n",allInputModels[model_ind]);
//
//	pFile = fopen (strcat(Clone_Dir, allInputModels[model_ind]), "rb");
//
...
//	if (result != lSize) {fputs ("Reading error",stderr); exit (3);}   // check gives integer comparison warning
	...
	while (toks!=NULL)	
	{
		switch(tstore)
		{
			...
			case 8: outerXBound=atof(toks); tstore=0; break;
			case 9: outerYBound=atof(toks); tstore=0; break;
			case 10: outerZBound=atof(toks); tstore=0; break;
...
void WriteGAInputFile(int num_cells, int num_params)
{
	...
	{	
		...
		for (int trip=0; trip < 3; trip++) {
			fprintf(GAfile, "double %f %f\n", /*0.0*/Footprint_xmin, outerXBound);
			fprintf(GAfile, "double %f %f\n", /*0.0*/Footprint_ymin, outerYBound);
			fprintf(GAfile, "double %f %f\n", Structure_zmin, outerZBound);
		}
	}
	...
	strcpy(Clone_Dir, Unique_Dir);	
	if (loadInitPopulation==0) {							   // whether to load population file
...
// SETUP CELL VERTICES 
//#####################################################################
// setupCell() is one of the first 3DPV functions written. it was formerly very long because we were still thinking about
...
void setupCell(Cell_3 &cel, double *coordinates, int cellIndex)  // note cell_index mainly for MODEL
{	
	...
	cel.vertices[0]->x = coordinates[cellIndex*9+0]; // get coordinate from global pointer to array modelCoordinates
	cel.vertices[0]->y = coordinates[cellIndex*9+1];
	cel.vertices[0]->z = coordinates[cellIndex*9+2];

	cel.vertices[1] = new Position;
	cel.vertices[1]->x = coordinates[cellIndex*9+3];
	cel.vertices[1]->y = coordinates[cellIndex*9+4];
	cel.vertices[1]->z = coordinates[cellIndex*9+5];

	cel.vertices[2] = new Position;
	cel.vertices[2]->x = coordinates[cellIndex*9+6];
	cel.vertices[2]->y = coordinates[cellIndex*9+7];
	cel.vertices[2]->z = coordinates[cellIndex*9+8];
...
// but it is actually still used, called right after setupCell for a solar cell.
...
//###################################################################################
void buildStructureFromCoordinates(Cell_3 structure_array[], double coordinates[])
{
	for (int i=0; i<numCells; i++)
	{
		setupCell(structure_array[i], coordinates, i);  // pass address of the ith element
	}
}

void retrieveCoordinatesFromStructure(double *coordinates, Cell_3 *cellArray) {
...
// changed by Marco, using equation for correction of Air Mass. 11 Dec 2010
	...
	double solar_variable = 1488*pow(0.7,(0.678/cos(amtheta)));  // then replace 2-> cos(spa.zenith!)
	...
	if (qwik_skip == -1) {
		...
		//	Edge *curr_edge;
			...
			for (double curTime=start_time; curTime<=end_time; curTime+=delta_time){ //JIN: looping over time intervals in one day
				...
				{
					refer_pt[0] = outerXBound/2; // these three set-variable lines could be outside this time loop
					refer_pt[1] = outerYBound/2;
					refer_pt[2] = outerZBound/2;
				}
				else
				{
...
//	totalSurfaceArea=0;
//	buildStructureFromCoordinates(cellOut, MODEL);
...
void GetZenithAngleDistribution(double coordinates[])
{
	// reset
...
//	modelPointer = coordinates; 		// set the pointer for SetupCell... function
	buildStructureFromCoordinates(cells_Array, coordinates);
...
void GetAzimuthAngleDistribution(double coordinates[])
{
	// reset
...
void Get2dAngleDistribution(double coordinates[])
{
...
//		modelPointer = x; // global model pointer simply points to this array "x" passed in
		buildStructureFromCoordinates(cellArray, x);                // store cells in cellArray. compose overall energy-collecting structure
...
void AlterFunnelWaist(double my_x[], int my_y, int my_z)
{
	...
	for (int xxj=1; xxj < numCells*9; xxj++)
	{
		...
		if ((xxj+2)%3==0) { // first, is this a y-value?
			if ((my_x[xxj-1]) == outerXBound/2) { // next, is the vertex's x value in the center?
				my_x[xxj] = (double)my_y;
			}	
		}

		if ((xxj+1)%3==0) { // first, is this a z-value?
			if ((my_x[xxj-2]) == outerXBound/2) { // next, is the vertex's x value in the center of structure?
...
//					rx+=outerXBound/2.0; //center shape at 5,5,5
//				    ry+=outerYBound/2.0;
//				    z+=outerZBound/2.0;
//				    m[3*i] = rx;
//				    m[3*i+1] = ry;
...
//	modelPointer = modelCoordinates;
//
//	retrieveCoordinatesFromFile(modelCoordinates, 0);
//
//	Cell_3 cellArray[numCells];
//
//	buildStructureFromCoordinates(cellArray, MODEL);
//
//	double day_energee=0;
//
//	day_energee = newCalculatePowerDayCycle(cellArray, modelPointer);   // calculate power at several instants in the day
...
//outerXBound, outerYBound and outerZBound must not have more than numSigFigs (default 6) digits after the decimal point
void genRandomStructure1(double *modelCoordinates, int numCells, double xBound, double yBound, double zBound,
	...
	for(int i = 0; i < numCells; i++) {
		for(int j = 0; j < 3; j++) {
			x = (rand() % (xIntBound+1) ) / numSigFigs + xOffset;
			y = (rand() % (yIntBound+1) ) / numSigFigs + yOffset;
			z = (rand() % (zIntBound+1) ) / numSigFigs + zOffset;
...
void genRandomStructure2(double *modelCoordinates, int numCells, double outerXBound, double outerYBound, double outerZBound,
		double innerXBound, double innerYBound, double innerZBound){
	int xIntBound = outerXBound*numSigFigs;
	int yIntBound = outerYBound*numSigFigs;
	int zIntBound = outerZBound*numSigFigs;

	float innerBoxCenter[3] = {outerXBound/2, outerYBound/2, innerZBound/2};
	float innerBoxHalfSizes[3] = {innerXBound/2, innerYBound/2, innerZBound/2};
	float triVerts[3][3];
	double x, y, z;

	for(int i = 0; i < numCells; i++) {
		//inside loop, a random triangular cell is being constructed
		loop:
			for(int j = 0; j < 3; j++) {
		...
		//if the cell is not outside the inner box, reconstruct it
		if(triBoxOverlap(innerBoxCenter, innerBoxHalfSizes, triVerts) == 1) goto loop;
		else {
			for(int j=0; j<3; j++) {
				modelCoordinates[9*i + 3*j] 	= triVerts[j][0];
				modelCoordinates[9*i + 3*j + 1] = triVerts[j][1];
				modelCoordinates[9*i + 3*j + 2] = triVerts[j][2];
			}
		}
	}
}

//the random initial structure generator for MC2
void genRandomStructure(double *modelCoordinates, int numAbsorbers, int numMirrors,
		double innerXBound, double innerYBound, double innerZBound, double outerXBound, double outerYBound, double outerZBound) {
	genRandomStructure1(modelCoordinates, numAbsorbers, innerXBound, innerYBound, innerZBound,
			(outerXBound-innerXBound)/2, (outerYBound-innerYBound)/2, 0);

	genRandomStructure2(modelCoordinates + 9*numAbsorbers, numMirrors, outerXBound, outerYBound, outerZBound,
			innerXBound, innerYBound, innerZBound);
}


bool MCShouldChange(double difference, double normalizer) {
	double prob = exp(difference/normalizer);

	double p = (rand() % ((int) numSigFigs + 1)) / numSigFigs;
...
//wrap-around version, for MC and mirrors in MC2
double newCoordinate1(double oldCoordinate, int dimension) {
	double newCoordinate;
	double min = oldCoordinate - MC_stepSize; //the theoretical lower limit of the new coordinate
	int intRange = 2*MC_stepSize*numSigFigs; //the distance btw the lower and the upper limits

	newCoordinate = min + (rand() % (intRange + 1)) / numSigFigs;

	//the new coordinate is too small
	if(newCoordinate < 0) {
		double length = std::abs(newCoordinate);

		switch(dimension) {
		//x
		{
		case 0:
			double ratio1 = length/outerXBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = outerXBound*(1-ratio2);
			break;
		}
		//y
		{
		case 1:
			double ratio1 = length/outerYBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = outerYBound*(1-ratio2);
			break;
		}
		//z
		{
		case 2:
			double ratio1 = length/outerZBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = outerZBound*(1-ratio2);
			break;
		}
		}
	} else { //check if the new coordinate is too large
		switch(dimension) {
		//x
		case 0:
			if(newCoordinate > outerXBound) {
				double length = newCoordinate - outerXBound;
				double ratio1 = length/outerXBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = outerXBound*ratio2;
			}
			break;

		//y
		case 1:
			if(newCoordinate > outerYBound) {
				double length = newCoordinate - outerYBound;
				double ratio1 = length/outerYBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = outerYBound*ratio2;
			}
			break;

		//z
		case 2:
			if(newCoordinate > outerZBound) {
				double length = newCoordinate - outerZBound;
				double ratio1 = length/outerZBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = outerZBound*ratio2;
			}
			break;
		}
	}
...
//	double newCoordinate;
//	double max;
//	double min = std::max(0.0, oldCoordinate - MC_stepSize);
//
//	switch(dimension) {
//	//x
//	case 0:
//		max = std::min(oldCoordinate + MC_stepSize, outerXBound);
//		break;
//	//y
//	case 1:
//		max = std::min(oldCoordinate + MC_stepSize, outerYBound);
//		break;
//	//z
//	case 2:
//		max = std::min(oldCoordinate + MC_stepSize, outerZBound);
//		break;
//	}
//
//	int intRange = (max - min)*numSigFigs;
//	newCoordinate = min + (rand() % (intRange + 1)) / numSigFigs;
//
//	return newCoordinate;
//}
...
//non-wrap-around version
//double newCoordinate(double oldCoordinate, int dimension) {
//	double newCoordinate;
//	double min, max;
//
//	switch(dimension) {
//	//x
//	case 0:
//		min = std::max((outerXBound - innerXBound)/2, oldCoordinate - MC_stepSize);
//		max = std::min(oldCoordinate + MC_stepSize, (outerXBound + innerXBound)/2);
//		break;
//	//y
//	case 1:
//		min = std::max((outerYBound - innerYBound)/2, oldCoordinate - MC_stepSize);
//		max = std::min(oldCoordinate + MC_stepSize, (outerYBound + innerYBound)/2);
//		break;
//	//z
//	case 2:
//		min = std::max(0.0, oldCoordinate - MC_stepSize);
//		max = std::min(oldCoordinate + MC_stepSize, innerZBound);
//		break;
//	}
//
//	int intRange = (max - min)*numSigFigs;
...
//wrap-around version, for absorbers in MC2
double newCoordinate(double oldCoordinate, int dimension) {
//	cout << dimension << ", " << oldCoordinate << ", ";

	//translate the old coordinate toward the origin
	switch(dimension) {
	case 0:
		oldCoordinate = oldCoordinate - (outerXBound - innerXBound)/2;
		break;
	case 1:
		oldCoordinate = oldCoordinate - (outerYBound - innerYBound)/2;
		break;
	case 2:
		break;
	}

	double min = oldCoordinate - MC_stepSize;
	int intRange = 2*MC_stepSize*numSigFigs;

	double newCoordinate = min + (rand() % (intRange + 1)) / numSigFigs;
	...
	if(newCoordinate < 0) {
		...
		{
		case 0:
			double ratio1 = length/innerXBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = innerXBound*(1-ratio2);
			break;
		}
		//y
		{
		case 1:
			double ratio1 = length/innerYBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = innerYBound*(1-ratio2);
			break;
		}
		//z
		{
		case 2:
			double ratio1 = length/innerZBound;
			double ratio2 = ratio1 - floor(ratio1);
			newCoordinate = innerZBound*(1-ratio2);
			break;
		}
		}
	} else { //check if the new coordinate is too large
		...
		//x
		case 0:
			if(newCoordinate > innerXBound) {
				double length = newCoordinate - innerXBound;
				double ratio1 = length/innerXBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = innerXBound*ratio2;
			}
			break;

		//y
		case 1:
			if(newCoordinate > innerYBound) {
				double length = newCoordinate - innerYBound;
				double ratio1 = length/innerYBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = innerYBound*ratio2;
			}
			break;

		//z
		case 2:
			if(newCoordinate > innerZBound) {
				double length = newCoordinate - innerZBound;
				double ratio1 = length/innerZBound;
				double ratio2 = ratio1 - floor(ratio1);
				newCoordinate = innerZBound*ratio2;
			}
			break;
		}
	}
	...
	switch(dimension) {
	case 0:
		newCoordinate = newCoordinate + (outerXBound - innerXBound)/2;
		break;
	case 1:
		newCoordinate = newCoordinate + (outerYBound - innerYBound)/2;
		break;
	case 2:
		break;
	}
...
//this method is for MC
void getNextCoordinates1(double *nextCoordinates, double *curCoordinates) {
	int selectedIndices[MC_numChanges];

	int range[numCoordinates];
	for(int i = 0; i < numCoordinates; i ++) {
		range[i] = i;
	}

	vector<double> v(range, range + numCoordinates);
	for(int i = 0; i < MC_numChanges; i ++) {
		int selection = rand() % v.size();
		selectedIndices[i] = v[selection];
		v.erase(v.begin()+selection);
	}

	copy(curCoordinates, curCoordinates + numCoordinates, nextCoordinates);
	for(int i = 0; i < MC_numChanges; i ++) {
		int index = selectedIndices[i];
		double oldCoordinate = nextCoordinates[index];
		nextCoordinates[index] = newCoordinate1(oldCoordinate, index % 3);
	}
}

//this method is for MC2
void getNextCoordinates(double *nextCoordinates, double *curCoordinates) {
	int range[numCoordinates];
	for(int i = 0; i < numCoordinates; i ++) {
		range[i] = i;
	}
	...
	if(MC_flag == "a" or MC_flag == "ma") {
		//indices of the selected to-be-changed coordinates
		int selectedIndices[MC_numChanges];

		//vector v dynamically tracks the indices of unselected coordinates
		vector<double> v(range, range + 9*numAbsorbers);
		for(int i = 0; i < MC_numChanges; i ++) {
			int selection = rand() % v.size();
			selectedIndices[i] = v[selection];
			v.erase(v.begin()+selection);
		...
		for(int i = 0; i < MC_numChanges; i ++) {
			int index = selectedIndices[i];
			double oldCoordinate = nextCoordinates[index];
			nextCoordinates[index] = newCoordinate(oldCoordinate, index % 3);
		}
	}

	if(MC_flag == "m" or MC_flag == "ma") {
		//indices of the selected to-be-changed coordinates
		int selectedIndices[MC_numChanges];

		//vector v dynamically tracks the indices of unselected coordinates
		vector<double> v(range + 9*numAbsorbers, range + numCoordinates);
		for(int i = 0; i < MC_numChanges; i ++) {
			int selection = rand() % v.size();
			selectedIndices[i] = v[selection];
			v.erase(v.begin()+selection);
		...
		for(int i = 0; i < MC_numChanges; i ++) {
			...
			int a = index/9;
			for(int j=0; j<3; j++) {
				triVerts[j][0] = nextCoordinates[9*a+3*j];
				triVerts[j][1] = nextCoordinates[9*a+3*j+1];
				triVerts[j][2] = nextCoordinates[9*a+3*j+2];
			}
			...
			loop:
				double newCoord = newCoordinate1(oldCoordinate, index % 3);
				triVerts[b][c] = newCoord;

			//just use the old coordinate if counter goes over 10
			if(counter <= 10) {
				//checks if the new triangle intersects the inner box
				if(triBoxOverlap(innerBoxCenter, innerBoxHalfSizes, triVerts) == 1) {
//								cout << index << ": " << newCoord << endl;
								counter++;
								goto loop;
							}
				else nextCoordinates[index] = newCoord;
			}
		}
	}
}
...
void outputCoordinates(string directory, int num, double *coordinates) {
	stringstream ss;
	ss << directory << "/" << num << ".txt";
	ofstream file(ss.str().data());

	for(int i = 0; i < numCells; i ++) {
		for(int j = 0; j < 9; j ++) {
			file << coordinates[9*i + j];
			if(j % 3 == 2) {
				file << " ";
			} else {
				file << ",";
...
int main(int argc, char *argv[])
{
    ...
    cout << "3DPV - hybrid Monce Carlo - version: " << softVersion << endl;
    cout << "Developed by the Grossman Group at MIT : http://zeppola.mit.edu"
    << endl << endl;
	string mainInputFile = argv[1];
	string outputDirectory = argv[2];
	int mode = atoi(argv[3]);
    
    // ****** (NF 2012/08/18) set flag for initial energy
    
    double initialEnFlag = 0.0;
...
//		numCoordinates=ReadStructureVertices_Count(0);
		double coordinates[9 * numCells];
		modelPointer = coordinates;
		retrieveCoordinatesFromFile(coordinates, 0);
		Cell_3 cellArray[numCells];
		buildStructureFromCoordinates(cellArray, coordinates);
		...
		double day_energee=0;
		day_energee = newCalculatePowerDayCycle(cellArray, modelPointer, efficiencies, reflIndices, true, true, outputDirectory);   // calculate power at several instants in the day


	} else if(mode == 1) { //fixed shape with cell efficiency and reflectivity specifications
...
//		numCoordinates=ReadStructureVertices_Count(0);
		...
		double efficiencies[numCells], refrIndices[numCells];
		...
		double day_energee=0;
		day_energee = newCalculatePowerDayCycle(cellArray, modelPointer, efficiencies, refrIndices, false, false, outputDirectory);   // calculate power at several instants in the day
...
//			numCoordinates=ReadStructureVertices_Count(0);
			...
			{
				printf("first %i\n", nm);
				retrieveCoordinatesFromFile(modelCoordinates, nm);
				printf("2nd %i\n", nm);
			...
			for (int i=0; i < GA_numDistinctModels; i++)
			{
				...
				genRandomStructure1(modelCoordinates, numCells, outerXBound, outerYBound, outerZBound, 0, 0, 0);
		...
		numCoordinates = 9 * numCells;
		double coordinates1[numCoordinates], coordinates2[numCoordinates];
		double *curCoordinates = coordinates1, *nextCoordinates = coordinates2, *tmpCoordinates;
		Cell_3 curCellArray[numCells], nextCellArray[numCells];
		double efficiencies[numCells], refrIndices[numCells];
		double bestTotalEnergy, curTotalEnergy, nextTotalEnergy;
 
        // ****** (NF 2012/08/18) define initial Total Energy
		...
		int numUpdates = 0;
		stringstream ss;
		string s1 = outputDirectory + "/uniform_update_profile.txt";
		ofstream uniformUpdateProfile(s1.data());
		ss << outputDirectory << "/real_update_profile.txt";
		ofstream realUpdateProfile(ss.str().data());
		ss.clear();

		if(loadInitPopulation) {
			retrieveCoordinatesFromFile(curCoordinates, 0); //0 because only a single fixed run
		} else {
			genRandomStructure1(curCoordinates, numCells, outerXBound, outerYBound, outerZBound, 0, 0, 0);
		}

		modelPointer = curCoordinates;
		buildStructureFromCoordinates(curCellArray, curCoordinates);

		//always read from specification file
		ifstream ifs(modelSpecDir.data());
		for(int i=0; i<numCells; i++) {
			ifs >> efficiencies[i];
			ifs >> refrIndices[i];
		...
		curTotalEnergy = newCalculatePowerDayCycle(curCellArray, modelPointer, efficiencies, refrIndices);
		bestTotalEnergy = curTotalEnergy;
		bestStep = 0;
        
        // ****** (NF 2012/08/19) set initial Total Energy
        
        if(initialEnFlag = 0) {
            initialTotalEnergy = curTotalEnergy;
        }
        
        // ******


		//default T-value is parameter1/parameter2, which is the correct setting for type 0 T-value
		MC_TValue = TValueParameter1/TValueParameter2;

		cout << 0 << "th step: " << curTotalEnergy << ", 1"<< endl;
		realUpdateProfile << 0 << "\t" << curTotalEnergy << endl;
		uniformUpdateProfile << numUpdates << "\t" << curTotalEnergy << endl;
		outputCoordinates(outputDirectory, numUpdates, curCoordinates);

//		double totalChange = 0;

		for(int i = 1; i < MC_maxSteps; i ++) {
			if(TValueType == 1) {
				//ab^t
				MC_TValue = TValueParameter1*std::pow(TValueParameter2, i);
			} else if(TValueType == 2) {
				//c / (a + ln(t))
				MC_TValue = TValueParameter2 / (TValueParameter1 + std::log(i));
			}

			getNextCoordinates1(nextCoordinates, curCoordinates);
			modelPointer = nextCoordinates;
			buildStructureFromCoordinates(nextCellArray, nextCoordinates);
			nextTotalEnergy = newCalculatePowerDayCycle(nextCellArray, modelPointer, efficiencies, refrIndices, true, true, outputDirectory);

//			totalChange += std::abs(nextTotalEnergy - curTotalEnergy);

			if(nextTotalEnergy >= curTotalEnergy) {
				numUpdates ++;
				tmpCoordinates = curCoordinates;
				curCoordinates = nextCoordinates;
				nextCoordinates = tmpCoordinates;
				curTotalEnergy = nextTotalEnergy;

				if(curTotalEnergy >= bestTotalEnergy) {
					bestTotalEnergy = curTotalEnergy;
					bestStep = numUpdates;
				}
                
                // ****** (NF 2012/08/18) set initial Total Energy
				...
				cout << i << "th step: " << curTotalEnergy << ", 1"<< endl;
				realUpdateProfile << i << "\t" << curTotalEnergy << endl;
				uniformUpdateProfile << numUpdates << "\t" << curTotalEnergy << endl;

				if(numUpdates % 100 == 0)
					outputCoordinates(outputDirectory, numUpdates, curCoordinates);
			} else if(MCShouldChange(nextTotalEnergy - curTotalEnergy, MC_TValue)) {
				numUpdates ++;
				tmpCoordinates = curCoordinates;
				curCoordinates = nextCoordinates;
				nextCoordinates = tmpCoordinates;
				...
				cout << i << "th step: " << curTotalEnergy << ", 0" << endl;
				realUpdateProfile << i << "\t" << curTotalEnergy << endl;
				uniformUpdateProfile << numUpdates << "\t" << curTotalEnergy << endl;
		...
		uniformUpdateProfile.close();
		realUpdateProfile.close();
//		cout << "average change is: " << (totalChange/MC_maxSteps) << endl;

        // ****** (NF 2012/08/19) Using a more descriptive output
        
//		cout << "acceptance rate: " << ((double) numUpdates / MC_maxSteps) << endl;
//		cout << "final energy: " << curTotalEnergy << endl;
//		cout << "(best step, best energy): " << "(" << bestStep << ", " << bestTotalEnergy << ")" << endl;
        ...
        // ****** (NF 2012/08/18) display initial energy
        
        cout << "initial energy: " << initialTotalEnergy << " WattSec : " <<initialTotalEnergy/3600000 << " kWh" << endl;
        
        // ******
        
        cout << "final energy: " << curTotalEnergy << " WattSec : " << curTotalEnergy/3600000 << " kWh" << endl;
        
        // ****** (NF 2012/08/18) percentage gain from optimization
        
        cout << "total energy gain from optimization: " << 100.0*(curTotalEnergy - initialTotalEnergy)/initialTotalEnergy << " %" << endl << endl;
        
		cout << "(best step, best energy): " << "(" << bestStep << ", " << bestTotalEnergy << " Ws, " << bestTotalEnergy/3600000 << " kWh)" << endl;
        
        // ******
        
        
	} else if(mode == 4) { //Monte Carlo with mirrors and absorbers separated
		//initialize random seed
		srand (time(NULL));
		...
		if(loadInitPopulation) {
			...
			genRandomStructure(curCoordinates, numAbsorbers, numMirrors,
					innerXBound, innerYBound, innerZBound, outerXBound, outerYBound, outerZBound);
		}
...
//		double totalChange = 0;
		...
		for(int i = 1; i < MC_maxSteps; i ++) {
			...
			getNextCoordinates(nextCoordinates, curCoordinates);
			modelPointer = nextCoordinates;
			buildStructureFromCoordinates(nextCellArray, nextCoordinates);
			nextTotalEnergy = newCalculatePowerDayCycle(nextCellArray, modelPointer, efficiencies, refrIndices, true, true, outputDirectory);
...
//		strcpy(allInputModels[0], Clone_Dir);				// finally, full location of the model..
//		numCoordinates=ReadStructureVertices_Count(0);
//		double modelCoordinates[numCoordinates];
//		retrieveCoordinatesFromFile(modelCoordinates, 0);
//
...
//		buildStructureFromCoordinates(cellArray2, MODEL);
...
//				//allPositions[ri][guy*2+0] = myRandom.random01() * (roof_xmax-outerXBound);
//				//allPositions[ri][guy*2+1] = myRandom.random01() * (roof_ymax-outerYBound);
//			}
