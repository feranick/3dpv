...
extern void globalEvaluate(double *values, double *objFunction,
		double *violation, double *penalty, int *noOfViolations);
...
Individual::Individual(void) {
	...
	if (globalSetup->gaType==SGA) {
		...
		if (!(globalSetup->loadPopulation))
			evaluateFitness(); // If Nsga, it's constructor will call it's evaluate.
	}
}
...
void Individual::evaluateFitness(void) {
	...
	for (ii = 0; ii < globalSetup->noOfDecisionVariables; ii++)
		values[ii] = chrom[ii];
	globalEvaluate(values, objFunction, violation, &penalty, &noOfViolations);
	delete [] values;
}
...
std::ostream &operator <<(std::ostream &out, Individual &x) {
	...
	for (int i=0; i<globalSetup->finalNoOfConstraints; i++)
		out << "Constraint Violation #" << i << " " << x.violation[i]
				<< std::endl;
	return out;
}
...
bool crowdingComp(const NsgaIndividual *guy1, const NsgaIndividual *guy2) {
	if ((guy1->rank < guy2->rank)|| ((guy1->rank == guy2->rank)
			&& (guy1->crowdingDistance > guy2->crowdingDistance)))
		return 1;
	else
		return 0;
}
...
bool constrTournComp(const Individual *guy1, const Individual *guy2,
	...
	if ((guy1->penalty <= ZERO) && (guy2->penalty > ZERO))
		...
		if (guy1Fitness > guy2Fitness)
			return 1;
	} else if (guy1->penalty < guy2->penalty)
		return 1;
	return 0;
}
...
int dominates(const NsgaIndividual *guy1, const NsgaIndividual *guy2) {
	...
	case NoConstraints:
		for (ii = 0; ii < globalSetup->finalNoOfObjectives; ii++) {
			if (guy1->fitness[ii] > guy2->fitness[ii])
				idomj = 1;
			else if (guy1->fitness[ii] < guy2->fitness[ii])
				jdomi = 1;
		}
		break;
	case Penalty:
	...
	case Tournament:
		if ((guy1->penalty <= ZERO)&&(guy2->penalty <= ZERO)) {
			...
			if (guy1->penalty < guy2->penalty)
				idomj = 1;
			else if (guy2->penalty < guy1->penalty)
				jdomi = 1;
		}
		break;
	default:
	...
	if ((idomj == 1)&&(jdomi == 1))
		return INONDOMJ;
	else if ((idomj == 1)&&(jdomi == 0))
		return IDOMJ;
	else if ((idomj == 0)&&(jdomi == 1))
		return JDOMI;
	else
		return INONDOMJ;
}

/// Penalty constraint comparison operator: Checks if indiviudal 1 is better than individual 2 if the constraint handling is through penalty method.
bool constrPenaltyComp(const Individual *guy1, const Individual *guy2) {
	if (*(globalSetup->typeOfOptimizations) == Maximization) {
		if (*(guy1->objFunction) - guy1->penalty > *(guy2->objFunction)
				- guy2->penalty)
			return 1;
	} else {
		if (*(guy1->objFunction) + guy1->penalty < *(guy2->objFunction)
...
bool individualComp(const Individual *guy1, const Individual *guy2) {
	if (*(globalSetup->typeOfOptimizations) == Maximization) {
		if (*(guy1->objFunction) > *(guy2->objFunction))
			return 1;
	} else {
		if (*(guy1->objFunction) < *(guy2->objFunction))
			return 1;
