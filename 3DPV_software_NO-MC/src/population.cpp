...
Population::Population() {   // usually population instatiated by constructor of GA only (in ga.cpp)
	// Things to be done:
	...
	case TournamentWOR:
		selection = new TournamentSelection(*((int *)globalSetup->selectionParameters), this);
		break;
	case SUS:
		selection = new StochasticUniversalSelection(this);
		break;
	case Truncation:
	...
	case OnePoint:
		crossover = new OneTwoPointCrossover(1);
		break;
	case TwoPoint:
		crossover = new OneTwoPointCrossover(2);
		break;
	case Uniform:
...
void Population::doRTS(void) {
	...
	for (ii = 0; ii < globalSetup->populationSize; ii++) {
		...
		for (jj = 0; jj < windowSize; jj++) {
			distance = 0.0;
			rndGuy = myRandom.boundedIntegerRandom(0,
					globalSetup->populationSize);
			for (kk = 0; kk < globalSetup->noOfDecisionVariables; kk++)
				distance += ((*tempGuys[rndGuy])[kk]-(*newGuys[ii])[kk])* ((*tempGuys[rndGuy])[kk]-(*newGuys[ii])[kk]);
			if (distance < minDist) {
				closest = rndGuy;
				minDist = distance;
			}
...
void Population::shareFitness() {
	...
	arraySize = ((globalSetup->populationSize)*(globalSetup->populationSize-1))
			/2;
	...
	for (ii = 0; ii < globalSetup->populationSize-1; ii++) {
		...
		for (jj = ii + 1; jj < globalSetup->populationSize; jj++) {
			...
			for (kk = 0; kk < globalSetup->noOfDecisionVariables; kk++) {
				...
				tempDist += ((phenoValue1-phenoValue2)
						*(phenoValue1-phenoValue2));
			}
			tempDist = sqrt(tempDist);
			if (tempDist < sigmaShare)
				sharingValue[ll] = 1.0 - pow((tempDist/(sigmaShare)),
						shareAlpha);
			else
				sharingValue[ll] = 0.0;
		...
		for (jj = 0; jj < ii; jj++) {
			transIndex = (globalSetup->populationSize)*jj + (ii-jj-1) - (jj*(jj
					+1)/2);
 ...
 ** This feature is disabled when Niching is on.
 ** Output:None
	...
	*avgobj = oneOverPopulationSize*(newGuys[0]->getObjective());
	for (ii = 1; ii < globalSetup->populationSize; ii++) {
		*avgobj += oneOverPopulationSize*(newGuys[ii]->getObjective());
		if (isBetter(newGuys[ii], newGuys[bestGuy]))
			bestGuy = ii;
		if (isBetter(newGuys[worstGuy], newGuys[ii]))
			worstGuy = ii;
	}
...
void Population::loadPopulationFromFile(void) {
	...
	for (ii = 0; ii < numToLoad; ii++) {
		...
		guys[ii]->loadIndividual(variableValues, objValues, constViolValues,
				penaltyValue);
		newGuys[ii]->loadIndividual(variableValues, objValues, constViolValues,
				penaltyValue);
	}
...
int Population::doLocalSearch(void) {
	...
	if ((globalSetup->gaType == SGA) && (globalSetup->localSearchMethod
			!= NoLocalSearch)) {

		for (ii = 0; ii < globalSetup->populationSize; ii++) {
			if (myRandom.flip(globalSetup->localSearchProbability)) {
				numEvals = localSearch->localSearcher(newGuys[ii], localGuy,
						freezeMask);
...
void Population::replacePopulation(void) {
	...
	if (fabs(globalSetup->replaceProportion - 1.0) <= ZERO) {
		...
		for (ii = startIndex; ii < globalSetup->populationSize; ii++)
			eliteGuys[ii]
					= new Individual(guys[randomArray1[globalSetup->populationSize - 1 - ii + startIndex]]);
...
///Perfrom selection
void Population::doSelect(void) {
	selection->select(mpool);
}
...
/***
 ...
 If the problem is a maximization and the constraint handling method is either a tournament method or 
 if there are no constraints, then the fitness is set equal to the objective value. If the constraint 
 handling method is penalty then the fitness is equal to the objective value minus the penalty due to 
 constraint violation. On the other hand, if the problem is a minimization one and the constraint handling 
 method is either a tournament method or if there are no constraints, then set the fitness equal to the negative 
 of the objective value. If the constraint handling method is the penalty method then the fitness is equal 
 to the negative of the objective value minus the penalty due to constraint violation.
 */
	...
	case SigmaScaling:
		...
		for (ii = 0; ii < globalSetup->populationSize; ii++) {
			if (fitStdDev >= 0.0001) {
				...
				scaledFitness = (newGuys[ii]->getFitness() - *avgfit + sigTerm)
						/sigTerm;
			} else
				scaledFitness = (newGuys[ii]->getFitness() + *avgfit)/(2.0
						*(*avgfit));
